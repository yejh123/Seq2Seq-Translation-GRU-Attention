class Beam(object):
    ...
    def step(self, prob, prev_beam, f_done):
        pre_score = prob.new_tensor(prev_beam.scores)   # [beam_size, ]
        score = prob + pre_score.unsqueeze(-1).expand_as(prob)  # [beam_size, num_voc]
        nbest_score, nbest_ix = score.view(-1).topk(self.beam_size, largest=False)  # [beam_size, ]
